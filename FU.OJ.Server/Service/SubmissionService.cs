...
namespace FU.OJ.Server.Service
{
    ...
    public class SubmissionService : ISubmissionService
    {
        ...
        )
        {
            ...
            string url =
                $"{_judgeServerUrl}/submissions/?base64_encoded={base64Encoded.ToString().ToLower()}&wait={wait.ToString().ToLower()}";
            ...
            foreach (var testFolder in testFolders)
            {
                ...
                if (File.Exists(inputFilePath) && File.Exists(outputFilePath))
                {
                    ...
                    var jsonContent = new StringContent(
                        JsonSerializer.Serialize(submissionRequest),
                        Encoding.UTF8,
                        "application/json"
                    );
            ...
            foreach (var token in tokenList)
            {
                ...
                if (
                    tokenResult == "Compilation Error"
                    || JsonDocument
                        .Parse(tokenResult)
                        .RootElement.GetProperty("status")
                        .GetProperty("description")
                        .GetString() == "Compilation Error"
                )
                {
                ...
                {
                    SubmissionId = submission.Id,
                    StatusDescription = JsonDocument
                        .Parse(tokenResult)
                        .RootElement.GetProperty("status")
                        .GetProperty("description")
                        .GetString(),
                    Time = JsonDocument
                        .Parse(tokenResult)
                        .RootElement.GetProperty("time")
                        .GetString(),
                    Memory =
                        JsonDocument
                            .Parse(tokenResult)
                            .RootElement.TryGetProperty("memory", out JsonElement memoryElement)
        ...
        public async Task<string> GetByTokenAsync(
            string token,
            bool base64Encoded = false,
            string fields = "stdout,time,memory,stderr,token,compile_output,message,status"
        )
        {
            string url =
                $"{_judgeServerUrl}/submissions/{token}?base64_encoded={base64Encoded.ToString().ToLower()}&fields={fields}";
            ...
            while (true)
            {
                ...
                var statusDescription = tokenResult
                    .GetProperty("status")
                    .GetProperty("description")
                    .GetString();
        ...
        public async Task<SubmissionView> GetByIdAsync(string id)
        {
            ...
            return new SubmissionView
            {
                ...
                Status = submission.Status,
                Results = submission
                    .Results.Select(result => new ResultView
                    {
        ...
        public async Task<(
            ...
            int totalPages
        )> GetAllSubmissionsAsync(Paging query)
        {
        ...
        public async Task<(
            List<SubmissionView> submissions,
            int totalPages
        )> GetAllSubmissionsBelongsUserAsync(Paging query, string username)
        {
