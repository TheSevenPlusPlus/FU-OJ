...
namespace FU.OJ.Server.Service
{
    ...
    public class TestcaseService : ITestcaseService
    {
        ...
        public async Task<string> CreateAsync(CreateTestcaseRequest request)
        {
            ...
            var existingTestCase = await _context.TestCases.FirstOrDefaultAsync(tc =>
            ...
            foreach (
                var dir in Directory.GetDirectories(Path.Combine(tempFolderPath, zipFolderName))
            )
            {
            ...
            var newTestCase = new TestCase
            {
                ...
                FolderPath =
                    finalFolderPath // Use FolderPath instead of folder_path
                ,
            };
        ...
        public async Task DeleteAsync(string problemCode)
        {
            var problem = await _context
                ...
                .FirstOrDefaultAsync(u => u.Code == problemCode); // Use Code instead of code
            if (problem == null)
                throw new Exception(ErrorMessage.NotFound);

            var testcase = await _context
                .TestCases.AsNoTracking()
                .FirstOrDefaultAsync(u => u.ProblemId == problem.Id); // Use ProblemId instead of problem_id
            if (testcase == null)
                throw new Exception("Test case not found");
