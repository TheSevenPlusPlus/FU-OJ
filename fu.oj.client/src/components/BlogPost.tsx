...
import { getBlogCommentById, createBlogComment, getLastUserComment, deleteBlogComment, updateBlogComment } from '../api/blogComment';
...
export default function BlogPost() {
    ...
    const handleCommentSubmit = async () => {
        try {
            ...
            if (newComment.trim()) {
                ...
                await createBlogComment({ content: newComment, username: userName, blogId: blog_id })
    ...
    const renderCommentPagination = () => {
        ...
        const endPage = Math.min(totalCommentPages, startPage + maxPagesToShow - 1)

        if (startPage > 1) {
            paginationItems.push(
                <Button key={1} onClick={() => handleCommentPageChange(1)} variant="outline" size="sm">
                    1
                </Button>
            )
            if (startPage > 2) {
        ...
        if (endPage < totalCommentPages) {
            ...
            paginationItems.push(
                <Button key={totalCommentPages} onClick={() => handleCommentPageChange(totalCommentPages)} variant="outline" size="sm">
                    {totalCommentPages}
                </Button>
            )
        }
            ...
            <div className="w-full max-w-3xl space-y-8">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        <DropdownMenuItem
                                            onClick={() => {
                                                const newContent = prompt("Update your comment:", comment.content)
                ...
                {showAlert && (
                    <Alert variant="destructive">
                        ...
                        <AlertDescription>
                            To prevent spam, you can only comment once per minute. Please try again later.
                        </AlertDescription>
                    </Alert>
                )}
            </div>
